...
func TestGateVideoGenerationRequestMapping(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			request: &schemas.BifrostVideoGenerationRequest{
				Model: "m",
				Input: &schemas.VideoGenerationInput{Prompt: "p", VideoURI: schemas.Ptr("https://cdn.example.com/source.mp4")},
			},
		...
		},
		{
			...
			request: &schemas.BifrostVideoGenerationRequest{
				Model:  "m",
				Input:  &schemas.VideoGenerationInput{Prompt: "p"},
				Params: &schemas.VideoGenerationParameters{Type: schemas.Ptr("upscale")},
			},
			wantErr: "type is not supported",
		},
		{
			name: "output format rejected",
			request: &schemas.BifrostVideoGenerationRequest{
				Model:  "m",
				Input:  &schemas.VideoGenerationInput{Prompt: "p"},
				Params: &schemas.VideoGenerationParameters{OutputFormat: schemas.Ptr("webm")},
			},
			wantErr: "output_format is not supported",
		},
		{
			name: "upscale factor rejected",
			request: &schemas.BifrostVideoGenerationRequest{
				Model:  "m",
				Input:  &schemas.VideoGenerationInput{Prompt: "p"},
				Params: &schemas.VideoGenerationParameters{UpscaleFactor: schemas.Ptr(2)},
			},
			wantErr: "upscale_factor is not supported",
		},
		{
			name: "target megapixels rejected",
			request: &schemas.BifrostVideoGenerationRequest{
				Model:  "m",
				Input:  &schemas.VideoGenerationInput{Prompt: "p"},
				Params: &schemas.VideoGenerationParameters{TargetMegapixels: schemas.Ptr(8)},
			},
			wantErr: "target_megapixels is not supported",
		},
		{
...
// TestGateProviderFailClosed 验证未开放能力全部在发网前拒绝。
func TestGateProviderFailClosed(t *testing.T) {
	...
	if _, err := provider.VideoEdit(ctx, key, &schemas.BifrostVideoEditRequest{}); err == nil {
		t.Fatal("VideoEdit must be unsupported")
	}
