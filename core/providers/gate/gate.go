...
//     Location（禁止自动重定向），第二跳用全新无 Authorization 请求、
//     SSRF-safe dialer 与 LargeResponseReader 流式返回。
package gate
...
type GateProvider struct {
	...
	downloadClient      *fasthttp.Client      // 下载第二跳专用 client：SSRF-safe dial、无 provider header
...
func NewGateProvider(config *schemas.ProviderConfig, logger schemas.Logger) (*GateProvider, error) {
	...
	// 下载第二跳专用 client：每次拨号都重新解析并校验目标 IP（公网才可连），
	// DNS-rebinding 无法绕过；不设置 ReadTimeout，视频体按流读取，空闲检测由
	// 调用方的 reader 链负责。
	ssrfDial := network.SSRFSafeDialContext(ssrfDialTimeout)
