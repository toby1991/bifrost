...
func ToGateVideoGenerationRequest(bifrostReq *schemas.BifrostVideoGenerationRequest) (*GateVideoGenerationRequest, error) {
	...
	if bifrostReq.Input.VideoURI != nil {
	...
	if bifrostReq.Params != nil {
		...
		if params.Type != nil {
			return nil, fmt.Errorf("type is not supported by gate provider")
		}
		if params.OutputFormat != nil {
			return nil, fmt.Errorf("output_format is not supported by gate provider")
		}
		if params.UpscaleFactor != nil {
			return nil, fmt.Errorf("upscale_factor is not supported by gate provider")
		}
		if params.TargetMegapixels != nil {
			return nil, fmt.Errorf("target_megapixels is not supported by gate provider")
		}
