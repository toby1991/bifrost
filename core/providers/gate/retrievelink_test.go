...
// TestGateRetrieveLinkExtraHeaders 锁定共享 HTTP helper 的透传规则：
// context 覆盖 provider 配置但不能覆盖服务端凭证，逐跳头不得透传。
func TestGateRetrieveLinkExtraHeaders(t *testing.T) {
	...
	ctx.SetValue(schemas.BifrostContextKeyExtraHeaders, map[string][]string{
		"X-Override":          {"context"},
		"X-Multi":             {"one", "two"},
		"Authorization":       {"Bearer untrusted-credential"},
		"Proxy-Authorization": {"untrusted-proxy-credential"},
		"Connection":          {"upgrade"},
		"Upgrade":             {"websocket"},
	})
	if got := provider.fetchContentDirectURL(ctx, testKey("gate-secret"), "j1"); got != location {
		t.Fatalf("fetchContentDirectURL() = %q, want %q", got, location)
	}
	got := <-headers
	...
	if !slices.Equal(got.Values("X-Multi"), []string{"one", "two"}) {
		t.Errorf("multi-value header = %q", got.Values("X-Multi"))
	}
	for _, key := range []string{"Proxy-Authorization", "Upgrade"} {
		if value := got.Get(key); value != "" {
			t.Errorf("hop-by-hop header %s leaked: %q", key, value)
		}
	}
	// 独立 transport 主动关闭连接，不接受 context 注入的 upgrade 语义。
	if value := got.Get("Connection"); value != "close" {
		t.Errorf("Connection = %q, want close", value)
	}
}
